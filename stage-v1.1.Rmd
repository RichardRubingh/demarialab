...
protein.coding.genes <-
  read.csv(
    "/home/ghost/Desktop/stage/parserpython/GRCh38_unique_genes",    
    header = T,
    as.is = T
  )
...
Note:
	...
	does not make sence. In other senarios it does make sence and we 
	will add them. 
...
replicative dataset. This procedure is similar for the onco-gene and ionizing radiation data-sets. 
...
the onco-gene dataset will be provided manually. 
...
Data from marthandan is also used seperatley. This is to show that 
there are some problems with this data-set. There were no problems 
with the other data-sets. So to ceep things consice :
...
Now everything seem to cluster fine on senescence. This prodecure was also done for the other datasets. 
But this was the only dataset where modiications were desired. To keep the information conciece , this
...
```{r pca13, cache=TRUE, out.width="900px"}
...
Create the pca plots in a single image( more compact ). 

```{r pca131, cache=TRUE, out.width="1000px"}



plots = lapply(c(1:nrow(pcaCombinations)), function(.x) 

qplot(scores[,pcaCombinations[.x,1]],scores[,pcaCombinations[.x,2]],main=paste("Combination",.x),
 shape=replictiveSen.cellStrain,
color=replictiveSen.condition) + theme(
...
do the prinsipal component analsysis to see if the data will cluster well
on senescence. 
...
(not possible before because redundacy in model) 
...
if the clustering on senesecnce is a sucess. 
...
In the PCA3 vs PCA4 we can now see a seperation between the senescence.
This is good this means the data can be distingioused for senesecne.
...
image where zero can be black. Otherwize the colors do not correspond 
correctley with the heatmapp. 
...
by a python script. We will save csv's for sorted on adjusted 
...
For the radiation dataset we have to relevel the factors since otherwize because of the 
design matrix consisting of a single factor the comparason will be made between irSen.conditionNo
and the rest. We have to relvel in order to generate the irSen.conditionir. 

Note:
	We still use the same GLM model even though it might be better to use the common method because of the 
	single factor in this calse. How ever we want to be steady on what we do and it is still possible to use 
...
later use this list to do tests the see how significant the values are shared amongsed the different 
datasets. 
...
We have to assing the genes to the column, we will do this by the 
...
This step is sitmilar to the one from edgeR before and the meta-analysis.
...
regarded to a membrande protein. 
...
We will then assing the genes as rownames. This can be handy for sorting functions etc. 
...
## get the genes for dds


```{r getddshfclfc, cache=TRUE}

plot(dds_M_replicative.unique$log2FoldChange)
percentage.trashold <- 3
values.use <- floor((length(dds_M_replicative.unique$log2FoldChange)/100)*percentage.trashold)
abline(v = values.use,col="red")
abline(v = length(dds_M_replicative.unique$log2FoldChange)-values.use,col="red")
dds_M_replicative.unique.highFC <- head(dds_M_replicative.unique,n = values.use)
dds_M_replicative.unique.lowFC <- tail(dds_M_replicative.unique,n = values.use)


plot(dds_M_oncogene.unique$log2FoldChange)
percentage.trashold <- 5
values.use <- floor((length(dds_M_oncogene.unique$log2FoldChange)/100)*percentage.trashold)
abline(v = values.use,col="red")
abline(v = length(dds_M_oncogene.unique$log2FoldChange)-values.use,col="red")
dds_M_oncogene.unique.highFC <- head(dds_M_oncogene.unique,n = values.use)
dds_M_oncogene.unique.lowFC <- tail(dds_M_oncogene.unique,n = values.use)

plot(dds_M_radiation.unique$log2FoldChange)
percentage.trashold <- 8
values.use <- floor((length(dds_M_radiation.unique$log2FoldChange)/100)*percentage.trashold)
abline(v = values.use,col="red")
abline(v = length(dds_M_radiation.unique$log2FoldChange)-values.use,col="red")
dds_M_radiation.unique.highFC <- head(dds_M_radiation.unique,n = values.use)
dds_M_radiation.unique.lowFC <- tail(dds_M_radiation.unique,n = values.use)

```
#~ 
#~ ## get the genes for dge
#~ 

```{r getdgehfclfc, cache=TRUE}

plot(dge_M_replicative.unique$logFC)
percentage.trashold <- 3
values.use <- floor((length(dge_M_replicative.unique$logFC)/100)*percentage.trashold)
abline(v = values.use,col="red")
abline(v = length(dge_M_replicative.unique$logFC)-values.use,col="red")
dge_M_replicative.unique.highFC <- head(dge_M_replicative.unique,n = values.use)
dge_M_replicative.unique.lowFC <- tail(dge_M_replicative.unique,n = values.use)

plot(dge_M_oncogene.unique$logFC)
percentage.trashold <- 3
values.use <- floor((length(dge_M_oncogene.unique$logFC)/100)*percentage.trashold)
abline(v = values.use,col="red")
abline(v = length(dge_M_oncogene.unique$logFC)-values.use,col="red")
dge_M_oncogene.unique.highFC <- head(dge_M_oncogene.unique,n = values.use)
dge_M_oncogene.unique.lowFC <- tail(dge_M_oncogene.unique,n = values.use)

plot(dge_M_radiation.unique$logFC)
percentage.trashold <- 8
values.use <- floor((length(dge_M_radiation.unique$logFC)/100)*percentage.trashold)
abline(v = values.use,col="red")
abline(v = length(dge_M_radiation.unique$logFC)-values.use,col="red")
dge_M_radiation.unique.highFC <- head(dge_M_radiation.unique,n = values.use)
dge_M_radiation.unique.lowFC <- tail(dge_M_radiation.unique,n = values.use)
...
```{r heatmapResults, cache=TRUE, out.width = "900px"}


VulcanoRNASEQPlotterDds <- function(x,xlim=c(-8,8)){

res <- x
# Make a basic volcano plot
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot\nDE genes DESeq", xlim=c(xlim)))
...
library(calibrate)
with(subset(res, padj<.01 & abs(log2FoldChange)>1), textxy(log2FoldChange, -log10(pvalue), labs=GeneName, cex=.8))


}


VulcanoRNASEQPlotterDge <- function(x,xlim=c(-8,8)){

res <- x

with(res, plot(logFC, -log10(PValue), pch=20, main="Volcano plot\nDE genes EdgeR", xlim=c(xlim)))
# Add colored points: red if padj<0.05, orange of log2FC>1, green if both)
with(subset(res, FDR<.01 ), points(logFC, -log10(PValue), pch=20, col="red"))
with(subset(res, abs(logFC)>1), points(logFC, -log10(PValue), pch=20, col="orange"))
with(subset(res, FDR<.01 & abs(logFC)>1), points(logFC, -log10(PValue), pch=20, col="green"))
# Label points with the textxy function from the calibrate plot
library(calibrate)
with(subset(res, FDR<.01 & abs(logFC)>1), textxy(logFC, -log10(PValue), labs=GeneName, cex=.8))
}
...
head(dge_M_replicative.unique[which(abs(dge_M_replicative.unique$padj) < 1e-12),])
